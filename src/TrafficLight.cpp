...
void TrafficLight::waitForGreen()
{
    ...
    while(true)
    {
        ...
        if (TrafficLightPhase::green == tlp)
            return;
...
void TrafficLight::cycleThroughPhases()
{
    ...
    {
        auto rdm = rand() % 6 + 4;
        std::this_thread::sleep_for(std::chrono::milliseconds(rdm*1000));
        auto tlp = TrafficLightPhase::red;
        if (TrafficLightPhase::red == getCurrentPhase())
        {
            setCurrentPhase(TrafficLightPhase::green);
            tlp = TrafficLightPhase::green;
        }
        else
